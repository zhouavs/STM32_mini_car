...
void ultrasonic_test() {
  ...
  Device_ultrasonic *pdu = NULL;
  err = Device_ultrasonic_find(&pdu, DEVICE_ULTRASONIC_1);
  if (err) goto print_err_tag;
  ...
  while (1) {
    uint32_t data = 0;
    err = pdu->ops->read(pdu, &data);
    if (err) goto print_err_tag;
    uint8_t str[50] = {0};
    snprintf((char *)str, 50, "distance: %" PRIu32 ".%02" PRIu32, data / 100, data % 100);
    err = pds->ops->fill_window(pds, 0xfff0);
    if (err) goto print_err_tag;
...
static errno_t init(void) {
  ...
  err = Device_config_ultrasonic_register_all_device();
  if (err) goto print_err_tag;
