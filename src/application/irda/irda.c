...
void irda_test() {
  ...
  Device_IRDA *pdi = NULL;
  err = Device_IRDA_find(&pdi, DEVICE_IRDA_1);
  if (err) goto print_err_tag;
  err = pdi->ops->init(pdi);
  if (err) goto print_err_tag;

  while (1) {
    uint8_t cmd = DEVICE_IRDA_CMD_NONE;
    err = pdi->ops->read(pdi, &cmd);
    if (err && err != ETIMEDOUT) goto print_err_tag;
    uint8_t str[20] = {0};
    snprintf((char *)str, 20, "irda cmd: %x", cmd);
    err = pds->ops->fill_window(pds, 0xfff0);
    if (err) goto print_err_tag;
...
static errno_t init(void) {
  ...
  err = Device_config_IRDA_register_all_device();
  if (err) goto print_err_tag;
